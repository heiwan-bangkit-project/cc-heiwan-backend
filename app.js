...
const param = req.query;
const id = param.id;
const queryStr = 'SELECT id, name, description, price, image, uuid FROM animals WHERE id = ? AND deleted_at IS NULL';
const values = [id];
conn.query(queryStr, values, (err, results) => {
  if(err){
    return res.status(500).json({
...
const certPath = '/etc/letsencrypt/live/heiwan-api.app';
const port = 3000
